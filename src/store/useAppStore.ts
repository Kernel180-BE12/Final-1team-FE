...
// // --- 연락처 타입 정의 (API 명세에 맞게 수정) ---
// export interface Contact {
//   id: number;
//   name: string;
//   phoneNumber: string;
//   email: string;
//   tag: string; // API 명세상 tag는 항상 존재
// }
...
// // 연락처 수정 시 사용할 타입
// export interface UpdateContactPayload extends Contact {
//   spaceId: number;
// }
...
//       set({ contacts: fetchedContacts, allTags, filteredContacts: filtered });
//     } catch (error) {
...
//   deleteAccount: async () => {
//     // 서버에 회원 탈퇴 API를 요청합니다.
//     await apiClient.delete('/user/delete');

//     // 탈퇴 성공 시, 스토어에 있는 logout 액션을 호출하여
//     // 모든 사용자 상태를 깨끗하게 지웁니다.
//     await get().logout();
//   },
...
//     // 백엔드 API가 여러 ID를 한번에 받는다고 가정
//     // 여러 번 호출하는 방식으로 구현
//     // await Promise.all(
//     //   selectedContactIds.map(id => 
//     //     apiClient.delete('/space/contact', {
//     //       data: { spaceId: currentSpace.spaceId, id }
//     //     })
//     //   )
//     // );
...
//         ids: selectedContactIds, // 'id'가 아닌 'ids' 배열을 전달
//       }
//     });
    
    
//     set({ selectedContactIds: [] }); // 선택 초기화
//     await get().fetchContacts();
//   },

//   toggleTagFilter: (tag) => {
//     const currentSelected = get().selectedTags;
//     // 이미 선택된 태그면 제거하고, 없으면 추가합니다.
//     const newSelected = currentSelected.includes(tag)
//       ? currentSelected.filter(t => t !== tag)
//       : [...currentSelected, tag];

//     const allContacts = get().contacts;
//     // 선택된 태그가 하나라도 있으면 필터링하고, 없으면 전체 목록을 보여줍니다.
//     const filtered = newSelected.length > 0
//       ? allContacts.filter(c => newSelected.includes(c.tag))
//       : allContacts;
...
//       set({ selectedContactIds: [] }); // 모두 선택된 상태면, 전체 해제
//     } else {
//       set({ selectedContactIds: allVisibleIds }); // 그렇지 않으면, 현재 보이는 모든 연락처 선택
//     }
//   },
