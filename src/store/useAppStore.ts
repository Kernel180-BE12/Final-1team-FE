...
interface MyActions {
  ...
  inviteSpaceMembers: (invitations: NewMemberInvitation[]) => Promise<InvitationResult>;
  updateSpaceMember: (memberId: number, data: { tag: string }) => Promise<void>;
  deleteSpaceMember: (memberId: number) => Promise<void>;
  deleteSelectedSpaceMembers: () => Promise<void>;
...
const actionsCreator: (
  ...
  fetchSpaceMembers: async () => {
    ...
    try {
      ...
      set({ spaceMembers: response.data || [], selectedMemberIds: [] }); // 멤버 목록 새로고침 시 선택 상태 초기화
    } catch (error) {
  ...
  inviteSpaceMembers: async (invitations) => {
    ...
    try {
      const invitationsWithAuthority = invitations.map(inv => ({ ...inv, authority: 'MEMBER' as const,}));

      const response = await apiClient.post<InvitationResult>(`/space-members/${currentSpaceId}/invitations`, invitationsWithAuthority);
  ...
  deleteSpaceMember: async (memberId) => {
    ...
    if (!currentSpace) throw new Error("스페이스가 선택되지 않았습니다.");
    
    try {
      await apiClient.delete(`/space-members/${memberId}`, {
        params: { spaceId: currentSpace.spaceId }
      });

      set((state) => ({
        spaceMembers: state.spaceMembers.filter((member) => member.id !== memberId),
  ...
  deleteSelectedSpaceMembers: async () => {
    const { currentSpace, selectedMemberIds } = get();
    if (!currentSpace || selectedMemberIds.length === 0) return;

    try {
      const memberIdsString = selectedMemberIds.join(',');

      await apiClient.delete(`/space-members/${memberIdsString}`, {
        params: { spaceId: currentSpace.spaceId }
      });

      set((state) => ({
        spaceMembers: state.spaceMembers.filter((member) => !selectedMemberIds.includes(member.id)),
        selectedMemberIds: [], // 선택 목록 초기화
      }));

      get().showSnackbar({ message: '선택한 멤버를 삭제했습니다.', severity: 'success' });
    } catch (error) {
      console.error("일괄 삭제 실패:", error);
      get().showSnackbar({ message: '일괄 삭제에 실패했습니다.', severity: 'error' });
      throw error;
  ...
  toggleMemberSelection: (id: number) => {
    const selectedIds = get().selectedMemberIds;
    const newSelectedIds = selectedIds.includes(id)
      ? selectedIds.filter(selectedId => selectedId !== id)
      : [...selectedIds, id];
    set({ selectedMemberIds: newSelectedIds });
  },
  
  toggleAllMembersSelection: () => {
    const { spaceMembers, selectedMemberIds, user } = get();
    const allSelectableIds = spaceMembers
      .filter(m => m.email !== user?.email)
      .map(m => m.id);
    
    const areAllSelected = allSelectableIds.length > 0 && allSelectableIds.every(id => selectedMemberIds.includes(id));

    if (areAllSelected) {
      set({ selectedMemberIds: selectedMemberIds.filter(id => !allSelectableIds.includes(id)) });
    } else {
      set({ selectedMemberIds: [...new Set([...selectedMemberIds, ...allSelectableIds])] });
    }
  },
